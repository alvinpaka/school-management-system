    ...
    <Sidebar>
        <template #header-title>
            <div class="flex items-center space-x-3">
                ...
                <span class="font-semibold text-warm-text dark:text-dark-text">Financial Dashboard</span>
            </div>
        </template>
        ...
        <div class="mx-auto max-w-7xl space-y-8 animate-fade-in-up">
                ...
                <div class="flex flex-col md:flex-row items-center md:items-end gap-6 lg:gap-8">
                    ...
                    <div class="text-center md:text-left space-y-2 lg:space-y-4">
                        <div class="flex items-center justify-center md:justify-start gap-2">
                            ...
                            <span class="text-[10px] font-black text-warm-muted dark:text-dark-muted uppercase tracking-widest">Active Session</span>
                        </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
                ...
                <div class="card-warm p-5 lg:p-6 rounded-3xl lg:rounded-[2.5rem] group hover:border-forest/30 transition-all duration-300">
                    ...
                    <p class="text-xs font-black text-warm-muted dark:text-dark-muted/80 uppercase tracking-widest mb-1">Total Revenue</p>
                    <h3 class="text-3xl font-black text-warm-text dark:text-dark-text tracking-tighter leading-none">{{ formatCurrency(financialSummary.totalRevenue) }}</h3>
                </div>
                ...
                <div class="card-warm p-5 lg:p-6 rounded-3xl lg:rounded-[2.5rem] group hover:border-terracotta/30 transition-all duration-300">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <Badge class="bg-muted border-0 text-warm-muted dark:text-dark-muted font-black text-[9px] uppercase tracking-widest">84% Rate</Badge>
                    </div>
                    <p class="text-xs font-black text-warm-muted dark:text-dark-muted/80 uppercase tracking-widest mb-1">Collected Fees</p>
                    <h3 class="text-3xl font-black text-warm-text dark:text-dark-text tracking-tighter leading-none">{{ formatCurrency(financialSummary.collectedFees) }}</h3>
                </div>
                ...
                <div class="card-warm p-5 lg:p-6 rounded-3xl lg:rounded-[2.5rem] group hover:border-amber/30 transition-all duration-300">
                    ...
                    <p class="text-xs font-black text-warm-muted dark:text-dark-muted/80 uppercase tracking-widest mb-1">Pending Fees</p>
                    <h3 class="text-3xl font-black text-warm-text dark:text-dark-text tracking-tighter leading-none">{{ formatCurrency(financialSummary.pendingFees) }}</h3>
                </div>
                ...
                <div class="card-warm p-5 lg:p-6 rounded-3xl lg:rounded-[2.5rem] group hover:border-destructive/30 transition-all duration-300">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <Badge class="bg-muted border-0 text-warm-muted dark:text-dark-muted font-black text-[9px] uppercase tracking-widest">-5.2%</Badge>
                    </div>
                    <p class="text-xs font-black text-warm-muted dark:text-dark-muted/80 uppercase tracking-widest mb-1">Total Expenses</p>
                    <h3 class="text-3xl font-black text-warm-text dark:text-dark-text tracking-tighter leading-none">{{ formatCurrency(financialSummary.totalExpenses) }}</h3>
                </div>
            </div>
                ...
                <div class="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6 lg:gap-8">
                    <div class="text-center md:text-left space-y-4">
                        <div class="flex items-center justify-center md:justify-start gap-4">
                            ...
                            <Badge class="bg-white/20 text-white border-white/20 font-black text-[9px] uppercase leading-none">Healthy Margin</Badge>
                        </div>
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 lg:gap-6">
                        ...
                        >
                            <div class="flex flex-col items-center text-center space-y-4">
                                <div class="w-14 h-14 rounded-2xl bg-white dark:bg-dark-bg border border-terracotta/20 flex items-center justify-center shadow-inner group-hover:bg-terracotta group-hover:text-white group-hover:border-terracotta transition-all">
                ...
                <div class="lg:col-span-2 space-y-6 lg:space-y-8">
                    <div class="card-warm rounded-3xl lg:rounded-[3rem] overflow-hidden">
                        ...
                        <div class="p-4 lg:p-8">
                            <div class="space-y-4">
                                <div v-for="(transaction, index) in recentTransactions" :key="index" class="flex items-center gap-4 lg:gap-6 p-4 lg:p-5 rounded-2xl lg:rounded-[2rem] bg-terracotta/5 border border-terracotta/20 group transition-all lg:hover:bg-white lg:hover:shadow-md">
                                    <div :class="[
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <div class="flex flex-wrap items-center justify-between gap-1 mb-1">
                                            <h4 class="text-base lg:text-lg font-black text-warm-text dark:text-dark-text tracking-tighter truncate">{{ transaction.title }}</h4>
                                            <Badge :class="[
                                        ...
                                        <div class="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                                            <p class="text-xs font-bold text-warm-muted dark:text-dark-muted uppercase tracking-wide truncate">{{ transaction.description }}</p>
                                            <div class="flex items-center gap-3 text-[10px] font-black uppercase text-warm-muted dark:text-dark-muted/80">
                        ...
                        <div class="p-8">
                            <div class="h-64 rounded-[2.5rem] bg-terracotta/5 border-2 border-dashed border-terracotta/20 flex items-center justify-center p-8">
                                <div class="text-center group">
                                    ...
                                    <p class="text-[10px] font-black text-warm-muted dark:text-dark-muted uppercase tracking-widest mb-6">Aggregate Institutional Performance</p>
                                    <div class="flex flex-wrap items-center justify-center gap-8">
                                        <div class="flex items-center gap-3">
                                            <div class="w-4 h-4 bg-forest rounded-lg shadow-sm"></div>
                                            <span class="text-xs font-black text-warm-muted dark:text-dark-muted uppercase">Rev: {{ formatCurrency(financialSummary.totalRevenue) }}</span>
                                        </div>
                                        <div class="flex items-center gap-3">
                                            <div class="w-4 h-4 bg-destructive rounded-lg shadow-sm"></div>
                                            <span class="text-xs font-black text-warm-muted dark:text-dark-muted uppercase">Exp: {{ formatCurrency(financialSummary.totalExpenses) }}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="space-y-4">
                                <div v-for="payment in pendingPayments" :key="payment.name" class="p-5 rounded-3xl lg:rounded-[2rem] bg-amber/5 border border-amber/20 group lg:hover:border-amber transition-all">
                                    <div class="flex items-start justify-between mb-4">
                                        <div>
                                            <h4 class="text-base font-black text-warm-text dark:text-dark-text leading-none mb-1 tracking-tight">{{ payment.name }}</h4>
                                            <p class="text-[10px] font-bold text-warm-muted dark:text-dark-muted uppercase tracking-widest">{{ payment.class }}</p>
                                        </div>
                                        <Badge class="bg-white dark:bg-dark-bg text-amber border border-amber/20 font-black text-[9px]">
                                            KES {{ (payment.amount / 1000).toFixed(0) }}K
                                        </Badge>
                                    </div>
                                    <div class="flex items-center gap-2 text-[10px] font-black uppercase text-warm-muted dark:text-dark-muted/80">
